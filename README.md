...
```bash
# 1. service設定ファイル作成（統合監視システム対応）
sudo tee /etc/systemd/system/kaggle-claude-mother.service > /dev/null <<EOF
[Unit]
Description=Kaggle Claude Mother - Autonomous Competition System with Integrated Monitoring
...
### 🤖 統合監視・自動修復システム（完全Python実装）

**30秒間隔の統合エージェント監視 + 60秒間隔のサービス監視・自動修復**

システムには統合監視エージェントが組み込まれており、エージェント監視とサービス監視を同時に実行：

#### 🔧 統合監視機能
- **✅ エージェント監視**: パフォーマンス指標・実験進捗・API使用量・GPU/メモリ使用率
- **✅ サービス監視**: systemctl状態確認・プロセス生存確認・ログエラー検出
- **✅ システム監視**: CPU/メモリ/ディスク使用量・システム負荷・リソース警告
- **✅ 自動修復**: エラーパターン検出時の自動修復（依存関係・メモリ・ネットワーク・サービス再起動）
- **✅ Issue統合管理**: 重複Issue防止・単一Issue内での継続レポート

#### 🛠️ 自動修復戦略
- **依存関係修復**: `ModuleNotFoundError`検出時の`uv sync --upgrade`自動実行
- **メモリ最適化**: メモリ使用量90%超過時のシステムキャッシュクリア・Python GC実行
- **サービス再起動**: プロセス停止検出時の自動サービス再起動（10分間隔制限付き）
- **ディスククリーンアップ**: 80%超過時の古いログ・一時ファイル自動削除
- **ネットワーク修復**: GitHub API接続問題時のレート制限チェック・待機

#### 📊 監視統計・レポート
- **30分間隔**: サービス状態レポート（稼働率・平均リソース・自動修復統計）
- **1時間間隔**: エージェント監視レポート（パフォーマンス・競技進捗・アラート統計）
- **リアルタイム**: エラー検出・自動修復時の即座通知・詳細レポート
...
import asyncio
agent = MonitorAgent('token', 'repo')
status = asyncio.run(agent.get_system_status())
print(f'稼働時間: {status[\"uptime_hours\"]:.1f}時間')
print(f'自動修復: {status[\"total_auto_repairs\"]}回実行')
print(f'サービス監視: {status[\"service_health_checks_count\"]}回チェック')
"
...
#### 🔄 シェルスクリプト依存完全除去
- **❌ 削除**: `monitoring_cycle.sh`（シェルスクリプト監視）
- **❌ 削除**: `run_monitoring_repair.sh`（cron実行スクリプト）
- **✅ 統合**: 全監視機能をPython `monitor_agent.py` に統合
- **✅ 重複防止**: Issue重複作成の完全防止
- **✅ 効率化**: 統一された監視システムによる管理効率向上
